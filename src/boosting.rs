...
pub fn fit_internal_rf_streaming(
    ...
    let x_owned = x.to_owned();
    let hist = if use_histogram {
        Some(HistogramData::build(&x_owned.view(), n_samples, n_features))
    } else {
        None
    };
    ...
    let collector: Mutex<Vec<(usize, TreeNode, Vec<f64>)>> =
        Mutex::new(Vec::with_capacity(n_estimators));
                    ...
                    if c >= min_trees && c > 1 {
                        let should_stop = if tol > 0.0 {
                            ...
                            for i in 0..n_samples {
                                ...
                                let half_w =
                                    z * (p * (1.0 - p) / k + z2 / (4.0 * k * k)).sqrt() / denom;
                            ...
                            for i in 0..n_samples {
                                pred_min = pred_min.min(mean[i]);
                                pred_max = pred_max.max(mean[i]);
                            }
    ...
    let n_features = if let Some(first_tree) = models.first().and_then(|rf| rf.trees.first()) {
        fn max_feature(node: &TreeNode) -> usize {
            ...
            if let Some(l) = &node.left {
                m = m.max(max_feature(l));
            }
            if let Some(r) = &node.right {
                m = m.max(max_feature(r));
            }
            m
        }
    ...
    for rf in models {
        for tree in &rf.trees {
            fn accumulate(node: &TreeNode, imp: &mut Vec<f64>, count: &mut f64) {
                ...
                if node.feature < imp.len() {
                    imp[node.feature] += node.samples as f64;
                    *count += node.samples as f64;
                }
                if let Some(l) = &node.left {
                    accumulate(l, imp, count);
                }
                if let Some(r) = &node.right {
                    accumulate(r, imp, count);
                }
            }
...
fn get_raw_predictions(
    ...
    for row in pred.iter_mut() {
        row.copy_from_slice(initial_pred);
    }
...
fn boost_class_forest<'a>(
    ...
    let class_models: Vec<&InternalRF> = (0..models.len())
        .step_by(n_out)
        .filter_map(|rs| models.get(rs + c))
        .collect();
...
fn boost_raw_cuda(
    ...
    for r in raw.iter_mut() {
        r.copy_from_slice(initial_pred);
    }
    for c in 0..n_out {
...
impl RFGBoostClassifier {
    ...
    fn fit(
        ...
        if self.n_classes == 2 {
            ...
            for _ in 0..self.n_estimators {
                ...
                let (rf, update, n_used) = self.fit_one_rf(
                    &x_owned, &targets, &newton_w, &config, &mut rng, min_trees, true,
                );
                self.trees_used.push(n_used);
                for i in 0..n_samples {
                    pred[i] += self.learning_rate * update[i];
                }
                self.models.push(rf);
            }
        } else {
            ...
            for c in 0..self.n_classes {
                let wc: f64 = (0..n_samples)
                    .filter(|&i| y_vec[i] as usize == c)
                    .map(|i| weights[i])
                    .sum();
                let p = if total_w > 0.0 {
                    (wc / total_w).clamp(1e-5, 1.0 - 1e-5)
                } else {
                    1e-5
                };
            ...
            // pred[sample][class]
            let mut pred: Vec<Vec<f64>> =
                (0..n_samples).map(|_| self.initial_pred.clone()).collect();
            ...
            for _ in 0..self.n_estimators {
                for c in 0..self.n_classes {
                    ...
                    for i in 0..n_samples {
                        pred[i][c] += self.learning_rate * update[i];
                    }
                    self.models.push(rf);
                }
            }
    ...
    fn predict(&self, x: PyReadonlyArray2<f64>, device: &str) -> PyResult<Vec<f64>> {
        if !self.is_fitted {
            return Err(PyValueError::new_err(
                "RFGBoostClassifier has not been fitted",
            ));
        }
        let x_arr = x.as_array();
        let raw = raw_dispatch(
            &self.models,
            &self.initial_pred,
            self.learning_rate,
            &x_arr.view(),
            self.initial_pred.len(),
            device,
        )?;
        ...
        if self.n_classes == 2 {
            Ok(raw
                .iter()
                .map(|p| if sigmoid(p[0]) > 0.5 { 1.0 } else { 0.0 })
                .collect())
        } else {
            Ok(raw
                .iter()
    ...
    fn predict_ci(&self, x: PyReadonlyArray2<f64>, alpha: f64) -> PyResult<Vec<Vec<f64>>> {
        ...
        if self.n_classes != 2 {
            return Err(PyValueError::new_err(
                "predict_ci only supports binary classification",
            ));
        }
        let x_arr = x.as_array();
        let n = x_arr.nrows();
    ...
    #[getter]
    fn trees_used(&self) -> Vec<usize> {
        self.trees_used.clone()
    }
...
impl RFGBoostRegressor {
    ...
    fn fit(
        ...
        for &ci in &cal_idx {
            ...
            for rf in &self.models {
                let tree_mean: f64 = rf.trees.iter().map(|t| traverse(t, &sample)).sum::<f64>()
                    / rf.trees.len() as f64;
        ...
        for _ in 0..self.n_estimators {
            ...
            if self.async_mode {
                ...
                self.trees_used.push(rf.trees.len());
                for i in 0..n_total {
                    pred_full[i] += self.learning_rate * update[i];
                }
                self.models.push(rf);
            } else {
                let rf = fit_internal_rf(
    ...
    fn predict(&self, x: PyReadonlyArray2<f64>, device: &str) -> PyResult<Vec<f64>> {
        if !self.is_fitted {
            return Err(PyValueError::new_err(
                "RFGBoostRegressor has not been fitted",
            ));
        }
        let x_arr = x.as_array();
        let raw = raw_dispatch(
    ...
    fn predict_ci(&self, x: PyReadonlyArray2<f64>, alpha: f64) -> PyResult<Vec<Vec<f64>>> {
        ...
        for rf in &self.models {
            ...
            for i in 0..n {
                pred_mean[i] += self.learning_rate * update[i];
            }
        }
        ...
        Ok((0..n)
            .map(|i| vec![pred_mean[i] - q, pred_mean[i] + q])
            .collect())
    }
...
pub struct RFGBoost {
    ...
    #[allow(dead_code)] // retained for introspection/serialization symmetry
    task: String,
}
...
impl RFGBoost {
    ...
    fn fit(
        ...
        if let Some(clf) = &mut self.clf {
            clf.fit(x, y, sample_weight)
        } else if let Some(reg) = &mut self.reg {
            reg.fit(x, y, sample_weight)
        } else {
            Err(PyValueError::new_err("Invalid state"))
        }
    }
    ...
    #[pyo3(signature = (x, device="cpu"))]
    fn predict(&self, x: PyReadonlyArray2<f64>, device: &str) -> PyResult<Vec<f64>> {
        if let Some(clf) = &self.clf {
            clf.predict(x, device)
        } else if let Some(reg) = &self.reg {
            reg.predict(x, device)
        } else {
            Err(PyValueError::new_err("Invalid state"))
        }
    }
    ...
    fn predict_ci(&self, x: PyReadonlyArray2<f64>, alpha: f64) -> PyResult<Vec<Vec<f64>>> {
        if let Some(clf) = &self.clf {
            clf.predict_ci(x, alpha)
        } else if let Some(reg) = &self.reg {
            reg.predict_ci(x, alpha)
        } else {
            Err(PyValueError::new_err("Invalid state"))
        }
    }
    ...
    #[getter]
    fn n_estimators(&self) -> usize {
        self.clf.as_ref().map_or_else(
            || self.reg.as_ref().map_or(0, |r| r.n_estimators),
            |c| c.n_estimators,
        )
    }
    #[getter]
    fn is_fitted(&self) -> bool {
        self.clf.as_ref().map_or_else(
            || self.reg.as_ref().is_some_and(|r| r.is_fitted),
            |c| c.is_fitted,
        )
    }
    #[getter]
    fn trees_used(&self) -> Vec<usize> {
        self.clf.as_ref().map_or_else(
            || {
                self.reg
                    .as_ref()
                    .map_or_else(Vec::new, |r| r.trees_used.clone())
            },
            |c| c.trees_used.clone(),
        )
    }
}
